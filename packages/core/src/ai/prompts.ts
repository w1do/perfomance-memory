...
- Фраза могла прийти из распознавания речи с ошибками в названиях. Исправляй только написание названий так, как они записаны в известных метках, папках и проектах пользователя (они ниже) или как их принято писать («клауды-коды», «клод код» → Claude Code; «чат gpt» → ChatGPT; «воблер», «жерлица» — уже верно); смысл не меняй и ничего не добавляй. Если слово непонятно — оставь как есть.
- statement — короткая чистая формулировка правила на языке пользователя, без слов «люблю/не люблю» (полярность хранится отдельно). Пример: «Грубые ответы ChatGPT». Формулировка должна читаться вместе с polarity: при dislike statement — то, что плохо («Код, который приходится долго править»), при like — то, что хорошо («Код сразу под продакшн»). Если пользователь говорит «не люблю, когда нет X» или «X обязательно» — это like про X.
- details — 1–2 фразы пояснения только из сказанного; если добавить нечего — null (не строка "null").
- polarity — like или dislike. Отрицание внутри фразы переворачивает оценку: «не люблю, когда нет тестов» → like, statement «Тесты…»; «люблю, когда без спешки» → like, statement «Без спешки».
- folder_path — путь папки. Сначала выбери подходящую существующую папку из дерева или из похожих папок. Новую создавай, только если ни одна не подходит. Форма: [Тема, (Подтема…), «Люблю» | «Не люблю»]; последний элемент совпадает с polarity. Имена папок — на языке пользователя, с заглавной буквы, без «/».
  ...
  «Разное» — только если сферу правила определить нельзя; еда, путешествия, хобби, быт — свои темы, даже если такой папки ещё нет.
...
- applies_to — к кому или чему относится правило: конкретный предмет, названный во фразе, в любой сфере, в нижнем регистре, пробелы — «_». Примеры: «ловить щуку на спиннинг» → ["щука", "спиннинг"]; «перелёты с детьми» → ["дети"]; «Claude Code пишет кратко» → ["claude_code"]; «тесты на Pest в Laravel» → ["pest", "laravel"]; «отели в Турции» → ["турция"]; «кинза в супе» → ["кинза"]; «не люблю рано вставать» → [] (нет предмета). Сначала бери подходящую из известных меток (список ниже); новую — только если ни одна не подходит. Только прямо названное во фразе. Файлы и пути в applies_to не пиши (им место в tags). Нет подходящих — [].
- Орфография: в statement, details, why и примерах исправь опечатки и ошибки распознавания, смысл и термины не меняй.
...
export function enrichUserMessage(input: EnrichInput): string {
  ...
  const targets = input.knownTargets.length ? input.knownTargets.join(', ') : '(нет)';
  const hint = input.projectHint ? `\nПроект указан явно: ${input.projectHint}` : '';
  return `Текущее дерево папок:\n${tree}\n\nТоп-3 похожих папки:\n${similar}\n\nИзвестные проекты: ${projects}\nИзвестные домены: ${input.knownDomains.join(', ')}\nИзвестные метки applies_to: ${targets}${hint}\n\nФраза пользователя:\n«${input.text}»`;
}
