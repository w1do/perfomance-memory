...
тогда отдаются **все** правила проекта и **все** общие правила, которые он наследует по своим меткам, темам
и областям, — без обрезки, раздел «# Общие правила, которые наследует проект»),
...
`applies_to` — к кому или чему относится правило, в любой сфере (`claude_code`, `php`, `спиннинг`, `дети`…).
Правило без `applies_to` — общее: относится к любой задаче своей области.