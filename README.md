...
| `DUPLICATE_SCORE` | `0.60` | Порог сходства формулировок для поиска старых дублей («Гигиена»); кандидатов проверяет модель |
...
2. Обогащение (`OPENAI_LLM_MODEL`, строго JSON): statement, details, polarity, folder_path, domain, project,
   ...
   Цели `applies_to`, которых нет во фразе, отбрасываются — сервис не выдумывает данные. `applies_to` — только из
   словаря (`TARGETS` в `text/glossary.ts`: php, laravel, claude_code…); файлы, пакеты и классы переезжают в теги,
   `me` и `any_ai` не хранятся. Опечатки: модель правит орфографию текста, а новая папка или тег, отличающиеся от
   существующих на 1–2 буквы, приклеиваются к ним («Деплои и CI» → «Деплой и CI»).
...
## Наследование: общее → стек → проект

Когда в задаче назван проект, `get_context_for_task` отдаёт целиком, без ранжирования и обрезки `top_k`:
все правила проекта и **все общие правила его стека**. Стек сервис выводит сам — из целей `applies_to` правил
проекта вместе с подразумеваемыми (`laravel` ⇒ `php`, `gitlab_ci` ⇒ `gitlab`, `git`). Общее правило входит, если
его цель есть в стеке, если оно лежит в папке технологии стека («Laravel») или если у него нет целей, а его
область — область стека (задачи, проекта или уже подошедших правил). Правила других проектов и общие правила
чужих технологий (правило про Next в проекте на Laravel) в ранжирование для этой задачи не попадают; жёсткие
правила приходят всегда.
...
- **Словарь `applies_to`** — см. «Поток данных»; старые правила приводятся к словарю автоматически при старте api.
...
| `distinct_from[]` | id правил, помеченных «не дубли» | keyword |
...
`GET /api/hygiene/duplicates` · `POST /api/hygiene/merge` (`{ keep, remove[] }`) ·
`POST /api/hygiene/distinct` (`{ ids[] }`) · `GET /api/status` (сервисы, активные модели и словарь `targets`, без секретов) · `GET /api/health`.
...
**Перенос данных с локального стека на сервер** (одна команда из корня проекта, нужен SSH к серверу Dokploy и
`docker` без sudo для этого пользователя):

```bash
scripts/push-dump.sh user@ваш-сервер        # спросит подтверждение; -y — без вопроса
```

Скрипт снимает свежую копию локально, копирует её на сервер по `scp`, находит там контейнер `backup` вашего стека,
восстанавливает копию и перезапускает `api` и `mcp`. Данные на сервере заменяются; их прежняя версия остаётся в
копиях серверного сервиса `backup`.
